...
function Libary:CreateWindow()
	...
	Mover.Name = "Mover"
	Mover.Parent = Sidebar
	Mover.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	Mover.BackgroundTransparency = 1.000
	Mover.Position = UDim2.new(0.800000131, 0, 0, 0)
	Mover.Size = UDim2.new(0, 23, 0, 23)
	Mover.ZIndex = 2
	Mover.Font = Enum.Font.SourceSansSemibold
	Mover.Text = "<"
	Mover.TextColor3 = Color3.fromRGB(199, 199, 199)
	Mover.TextScaled = true
	Mover.TextSize = 14.000
	Mover.TextWrapped = true
	--<< Properties >>--
	--<< Scripts >>--
	Mover.MouseButton1Down:Connect(function()
		if SideBarDB == false then
			SideBarDB = true
			if SidebarOpen == true then
				SidebarOpen = false
				TweenService:Create(Sidebar, TweenInfo.new(0.6, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Position = UDim2.new(-0.102,0,0.5,0)}):Play()
				TweenService:Create(Mover, TweenInfo.new(0.6), {Position = UDim2.new(1,0,0,0), Rotation = 180}):Play()
				TweenService:Create(Script, TweenInfo.new(0.6, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Position = UDim2.new(0.5,0,0.5,0), Size = UDim2.new(1,0,1,0)}):Play()
				wait(0.2)
				wait(0.1)
				...
				SidebarOpen = true
				TweenService:Create(Sidebar, TweenInfo.new(0.6, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Position = UDim2.new(0.102,0,0.5,0)}):Play()
				TweenService:Create(Mover, TweenInfo.new(0.6), {Position = UDim2.new(0.8,0,0,0), Rotation = 0}):Play()
				TweenService:Create(Script, TweenInfo.new(0.6, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Position = UDim2.new(0.602134585, 0, 0.5, 0), Size = UDim2.new(0.794088602, 0, 1, 0)}):Play()
				wait(0.2)
				SidebarOpen = true
				wait(0.1)
				SideBarDB = false
...
function Libary:CreateTab(TabName)
	...
	Window.Position = UDim2.new(0.602134585, 0, 0.5, 0)
	Window.Size = UDim2.new(0.794088602, 0, 1, 0)
	...
	function Lib:CreateSlider(SliderName, min, max, callback)
	...
	--<< Scripts >>--
		...
		UserInputService.InputChanged:Connect(function()
			if Dragging then
				local MousePos = UserInputService:GetMouseLocation()+Vector2.new(0,0)
